...
const notInFirstArray = (forbiddenValues, arr) => {
  // Solution code here...
  const newArr=arr.filter((ele)=> !forbiddenValues.includes(ele))
...
const getBaseStatGreaterThan = (arr, minBaseStat) => {
  // Solution code here...
  const newArr=arr.filter((ele)=> ele.baseStat>minBaseStat)
  return newArr;
};
...
const getCharactersWithoutChildren = (arr) => {
  const arr2=[];
  const newArr=arr.filter((ele)=>!ele.children&&ele.children==null)
  // {
  //   const obj={
  //     name: ele.name,
  //     spouse: ele.spouse,
