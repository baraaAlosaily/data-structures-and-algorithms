...
const forEachTwoToThe = (arr) => {
  // Solution code here...
  let myArrat=[];
  arr.forEach(ele=>myArrat.push(Math.pow(2,ele)))
  return myArrat
};
...
const charCode = (arr) => {
  // Solution code here...
return arr.map(ele=>ele.charCodeAt(0))
};
...
const extractStats = (arr) => {
  // Solution code here...
  return arr.map(ele=>`{${name[Object]}:${ele.stat.name}, total: ${ele.effort+ele.baseStat}} `)
};
